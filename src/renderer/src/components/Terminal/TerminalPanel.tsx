...
function getTerminalTheme(terminalThemeId: string, appTheme: string) {
  // If a specific terminal theme is selected, use it
  if (terminalThemeId && TERMINAL_THEMES[terminalThemeId]) {
    return TERMINAL_THEMES[terminalThemeId]
  }
  // Fallback to app theme
  return appTheme === 'light' ? TERMINAL_THEMES['catppuccin-latte'] : TERMINAL_THEMES['catppuccin-mocha']
}
...
export function TerminalPanel() {
  ...
  const setupTerminal = useCallback((container: HTMLDivElement): Promise<{ terminal: Terminal; fitAddon: FitAddon; observer: ResizeObserver }> => {
    return new Promise((resolve) => {
      ...
      const terminal = new Terminal({
        ...
        theme: getTerminalTheme(settings.terminalTheme, settings.theme),
  ...
  useEffect(() => {
    if (termRef.current) {
      termRef.current.options.theme = getTerminalTheme(settings.terminalTheme, settings.theme)
    }
  }, [settings.theme, settings.terminalTheme])
